...
class EncoderImage(nn.Module):
	...
	def __init__(self, opt):
		...
		self.smooth_7 = nn.Conv2d(2048, 2048, 3, 1, 1)
		self.smooth_14 = nn.Conv2d(1024, 1024, 3, 1, 1)
		self.smooth_28 = nn.Conv2d(512, 512, 3, 1, 1)
		self.down2 = nn.AvgPool2d(2)
		self.latlayer_1427 = nn.Conv2d(1024, 2048, 1, 1, 0)
		self.latlayer_28214 = nn.Conv2d(512, 1024, 1, 1, 0)
		self.latlayer_7214 = nn.Conv2d(2048, 1024, 1, 1, 0)
		self.latlayer_14228 = nn.Conv2d(1024, 512, 1, 1, 0)
	...
	def forward(self, images):
		...
		out_28 = out_28.view(-1, 512, 28, 28).type(self.dtype)
		out_7_final = out_7x7
		out_14_final = (self.latlayer_7214(out_7x7) + self.down2(out_14))
		out_28_final = (self.latlayer_14228(out_14) + self.down2(out_28))
		out_28_final = self.smooth_28(out_28_final)
		out_14_final = self.smooth_14(out_14_final)
		if self.gradcam:
			out_7_final.requires_grad_(True)

			self.register_activations(out_7_final)

			h = out_7_final.register_hook(self.activations_hook)


		out = self.gemp(out_7_final).view(-1, self.pool_dim)
		out = self.fc(out)
		out = l2norm(out)
...
class EncoderImage4(nn.Module):

	def __init__(self, opt):
		super(EncoderImage4, self).__init__()
	...
	def forward(self, images):
		...
		out_14 = out_14.view(-1, 1024, 14, 14).type(self.dtype)
		out_28 = out_28.view(-1, 512, 28, 28).type(self.dtype)
		out_7_temp = self._upsample_add(self.latlayer_1427(out_14), out_7x7)
		out_14_temp = self._upsample_add(self.latlayer_28214(out_28), out_14)
		out_28_temp = out_28
		out_14_temp = self._upsample_add(self.latlayer_7214(out_7_temp), out_14_temp)
		out_28_temp = self._upsample_add(self.latlayer_14228(out_14_temp), out_28_temp)
		out_7_final = self._upsample_add(self.latlayer_1427(out_14_temp), out_7_temp)
		out_14_final = self._upsample_add(self.latlayer_28214(out_28_temp), out_14_temp)
		out_28_final = out_28_temp
		out_7_final = out_7_final
		out_14_final = self.smooth_14(out_14_final)
		out_28_final = self.smooth_28(out_28_final)
