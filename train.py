...
import pickle
import torch
from loss import l2norm, ContrastiveLoss_rec, ContrastiveLoss_rec3
from option import parser, verify_input_args
import data
from vocab import Vocabulary 
from BSCN_model import BSCN
...
def train_model(epoch, data_loader, model, criterion, criterion_contraloss_7, criterion_contraloss_14, criterion_contraloss_28, criterion_contraloss2_7, criterion_contraloss2_14, criterion_contraloss2_28,optimizer, args):
	...
	for itr, data in enumerate(data_loader):
		...
		scores, scores_14, scores_28, A_IS_all_t, A_IS_all_t_14, A_IS_all_t_28, A_EM_all_t, A_EM_all_t_14, A_EM_all_t_28, A_IS_all_t_rev, A_IS_all_t_14_rev, A_IS_all_t_28_rev, m, Tr_m = model.forward_broadcast(img_src, img_trg, txt, txt_len)
		...
		loss = criterion(scores) + 1.0 * criterion(scores_14) + 1.0 * criterion(scores_28)

   
    
	
		loss_raw_contra_all = 1.0 * criterion_contraloss_7(A_IS_all_t, A_EM_all_t, m, Tr_m) + 1.0 * criterion_contraloss_14(A_IS_all_t_14, A_EM_all_t_14, m, Tr_m) \
						   + 0.3 * criterion_contraloss_28(A_IS_all_t_28, A_EM_all_t_28, m, Tr_m)
		loss_contra_all = 1.0 * criterion_contraloss2_7(A_IS_all_t, A_EM_all_t, m, Tr_m) + 1.0 * criterion_contraloss2_14(A_IS_all_t_14, A_EM_all_t_14, m, Tr_m) \
						   + 0.5 * criterion_contraloss2_28(A_IS_all_t_28, A_EM_all_t_28, m, Tr_m)
		loss = loss + 0.7 * loss_contra_all + 0.3 * loss_raw_contra_all
...
def main():
	...
	if args.model_version == "TIRG":
		...
		model = BSCN(vocab.word2idx, args)
	...
	criterion_contraloss = ContrastiveLoss_rec3(args, margin=0.4)
	criterion_contraloss_14 = ContrastiveLoss_rec3(args, margin=0.3)
	criterion_contraloss_28 = ContrastiveLoss_rec3(args, margin=0.2)
	criterion_contraloss2 = ContrastiveLoss_rec(args, margin=0.4)
	criterion_contraloss2_14 = ContrastiveLoss_rec(args, margin=0.3)
	criterion_contraloss2_28 = ContrastiveLoss_rec(args, margin=0.2)
	...
	for epoch in range(args.num_epochs):
		...
		if epoch != 0 and epoch < 6 and epoch % args.step_lr == 0:
			for g in optimizer.param_groups:
				print("Learning rate: {} --> {}\n".format(g['lr'], g['lr']*args.gamma_lr))
				g['lr'] *= args.gamma_lr
		if epoch != 0 and epoch >= 6 and epoch % (args.step_lr // 2) == 0:
			for g in optimizer.param_groups:
				print("Learning rate: {} --> {}\n".format(g['lr'], g['lr']*args.gamma_lr))
				g['lr'] *= args.gamma_lr

		train_model(epoch, trn_loader, model, criterion_bcloss, criterion_contraloss, criterion_contraloss_14, criterion_contraloss_28, criterion_contraloss2, criterion_contraloss2_14, criterion_contraloss2_28, optimizer, args)
